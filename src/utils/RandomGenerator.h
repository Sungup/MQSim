...
namespace Utils
{
  ...
  force_inline double
  RandomGenerator::FloatRandom()
  {
    return rand.NextDouble();
  }

  force_inline uint32_t
  RandomGenerator::Get_uint(uint32_t max_value)
  {
    return uint32_t(FloatRandom() * (max_value + 1));
  }

  force_inline int32_t
  RandomGenerator::Get_int(int32_t max_value)
  {
    return int32_t(FloatRandom() * (max_value + 1));
  }

  force_inline double
  RandomGenerator::Uniform(double a, double b)
  {
   ...
   */
  force_inline uint32_t
  RandomGenerator::Uniform_uint(uint32_t m, uint32_t n)
  {
    return m + (uint32_t)((n - m + 1.0) * FloatRandom());
  }

  force_inline int64_t
  RandomGenerator::Uniform_long(int64_t m, int64_t n)
  {
    return m + (int64_t)((double)(n - m + 1) * FloatRandom());
  }

  force_inline uint64_t
  RandomGenerator::Uniform_ulong(uint64_t m, uint64_t n)
  {
   ...
   */
  force_inline double
  RandomGenerator::Erlang(double x, double s)
  {
    int64_t i, k;
    double z;
    z = x / s;
   ...
   */
  force_inline double
  RandomGenerator::HyperExponential(double x, double s)
  {
    if (s < x)
   ...
   */
  force_inline double
  RandomGenerator::Normal(double x, double s)
  {
    double v1, v2, w, z1;
    if (Normal_z2 != 0.0)
    {
   ...
   */
  force_inline double
  RandomGenerator::LogNormal(double x, double s)
  {
   ...
   */
  force_inline int64_t
  RandomGenerator::Geometric0(double x)
  {
    double p = 1 / x;
    int64_t i = 0;
    while (FloatRandom() < p)
  ...
  force_inline int64_t
  RandomGenerator::Geometric1(double x)
  {
   ...
   */
  force_inline double
  RandomGenerator::Hyper_geometric(double x, double s)
  {
    double sqrtz = s / x;
   ...
   */
  force_inline int64_t
  RandomGenerator::Binomial(int64_t n, double u)
  {
    int64_t k = 0;
    for (long i = 0; i < n; i++)
      if (FloatRandom() < u)
   ...
   */
  force_inline int64_t
  RandomGenerator::Poisson(double x)
  {
    double b = exp(-x);
    int64_t k = 0;
    double p = 1;
   ...
   */
  force_inline double
  RandomGenerator::Weibull(double alpha, double beta)
  {
   ...
   */
  force_inline double
  RandomGenerator::Pareto(double alpha, double beta)
  {
   ...
   */
  force_inline double
  RandomGenerator::Inverse(double min, double max)
  {
   ...
   */
  force_inline double
  RandomGenerator::Triangular(double min, double middle, double max)
  {
    double y = FloatRandom();
    if (y <= (middle - min) / (max - min)) {
      return min + sqrt(y*max*middle - y*max*min - y*min*middle + y*min*min);
    } else {
      return max - sqrt(max*max
                         + y*max*middle - y*max*max - y*min*middle + y*max*min
                         - max*min - max*middle + min*middle);
    }
  }
}
