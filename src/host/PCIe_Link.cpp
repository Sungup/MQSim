  ...
  void PCIe_Link::Deliver(PCIe_Message* message)
  {
    ...
    case PCIe_Destination_Type::HOST://Message from SSD device to the host
      ...
      if (Message_buffer_toward_root_complex.size() > 1)//There are active transfers
        return;
      sim->Register_sim_event(sim->Time() + estimate_transfer_time(message), this, (void*)(intptr_t)PCIe_Destination_Type::HOST, static_cast<int>(PCIe_Link_Event_Type::DELIVER));
      break;
    case PCIe_Destination_Type::DEVICE://Message from Host to the SSD device
      ...
      if (Message_buffer_toward_ssd_device.size() > 1)
        return;
      sim->Register_sim_event(sim->Time() + estimate_transfer_time(message), this, (void*)(intptr_t)PCIe_Destination_Type::DEVICE, static_cast<int>(PCIe_Link_Event_Type::DELIVER));
      break;
    default:
      break;
    }
  ...
  void PCIe_Link::Execute_simulator_event(MQSimEngine::SimEvent* event)
  {
    ...
    case PCIe_Destination_Type::HOST:
      ...
      if (Message_buffer_toward_root_complex.size() > 0)//There are active transfers
        sim->Register_sim_event(sim->Time() + estimate_transfer_time(Message_buffer_toward_root_complex.front()),
