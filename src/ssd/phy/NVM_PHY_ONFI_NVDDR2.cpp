    ...
    WaitingCopybackWrites(ChannelCount)
{
  for (uint32_t ch = 0; ch < channel_count; ++ch) {
    __spec.emplace_back(params.Flash_Channel_Width,
                                                  ...
                                                  NvmTransactionFlash* tr)
{
  ...
  sim->Register_sim_event(sim->Time() + NVDDR2DataOutTransferTime(tr->Data_and_metadata_size_in_byte, __spec[tr->Address.ChannelID]),
                          this, &dieBKE, (int)NVDDR2_SimEventType::READ_DATA_TRANSFERRED);

  tr->STAT_transfer_time += NVDDR2DataOutTransferTime(tr->Data_and_metadata_size_in_byte, __spec[tr->Address.ChannelID]);
...
{
  ONFI_Channel_Base& target_channel = channels[bookKeepingEntry.__active_transactions.front()->Address.ChannelID];
...
{
  return NVDDR2DataInTransferTime(transaction->Data_and_metadata_size_in_byte, __spec[transaction->Address.ChannelID]);
}
...
{
  ONFI_Channel_Base& target_channel = channels[transaction_list.front()->Address.ChannelID];
  auto& target_spec = __spec[transaction_list.front()->Address.ChannelID];
  ...
  * then something illegarl is happening*/
  ...
  if (!dieBKE->free())
  {
    if (transaction_list.front()->SuspendRequired)
    {
      ...
      case Transaction_Type::WRITE:
        ...
        suspendTime = target_spec.ProgramSuspendCommandTime + targetChip.GetSuspendProgramTime();
        break;
      case Transaction_Type::ERASE:
        __stats.IssuedSuspendEraseCMD++;
        suspendTime = target_spec.EraseSuspendCommandTime + targetChip.GetSuspendEraseTime();
        break;
      default:
  ...
  case Transaction_Type::READ:
    ...
    for (auto tr: transaction_list)
      tr->STAT_transfer_time += target_spec.ReadCommandTime[transaction_list.size()];

    if (chipBKE->OngoingDieCMDTransfers.empty())
    {
      targetChip.StartCMDXfer();
      chipBKE->Status = ChipStatus::CMD_IN;
      chipBKE->Last_transfer_finish_time = sim->Time() + suspendTime + target_spec.ReadCommandTime[transaction_list.size()];
      sim->Register_sim_event(chipBKE->Last_transfer_finish_time, this, dieBKE, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED);
    }
    else
    {
      dieBKE->__die_interleaved_time = suspendTime + target_spec.ReadCommandTime[transaction_list.size()];
  ...
  case Transaction_Type::WRITE:
    if (((NvmTransactionFlashWR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE)
    {
      ...
      for (auto tr: transaction_list) {
        auto tr_transfer_time = NVDDR2DataInTransferTime(tr->Data_and_metadata_size_in_byte, target_spec);

        tr->STAT_transfer_time += target_spec.ProgramCommandTime[transaction_list.size()] + tr_transfer_time;
      ...
      if (chipBKE->OngoingDieCMDTransfers.empty()) {
        ...
        chipBKE->Last_transfer_finish_time = sim->Time() + suspendTime + target_spec.ProgramCommandTime[transaction_list.size()] + data_transfer_time;
        ...
        dieBKE->__die_interleaved_time = suspendTime + target_spec.ProgramCommandTime[transaction_list.size()] + data_transfer_time;
        chipBKE->Last_transfer_finish_time += dieBKE->__die_interleaved_time;
      }
  ...
  case Transaction_Type::ERASE:
    ...
    for (auto tr : transaction_list)
      tr->STAT_transfer_time += target_spec.EraseCommandTime[transaction_list.size()];

    if (chipBKE->OngoingDieCMDTransfers.empty())
    {
      targetChip.StartCMDXfer();
      chipBKE->Status = ChipStatus::CMD_IN;
      chipBKE->Last_transfer_finish_time = sim->Time() + suspendTime + target_spec.EraseCommandTime[transaction_list.size()];
      sim->Register_sim_event(chipBKE->Last_transfer_finish_time, this, dieBKE, (int)NVDDR2_SimEventType::ERASE_SETUP_COMPLETED);
    }
    else
    {
      dieBKE->__die_interleaved_time = suspendTime + target_spec.EraseCommandTime[transaction_list.size()];
      chipBKE->Last_transfer_finish_time += dieBKE->__die_interleaved_time;
    }
    chipBKE->OngoingDieCMDTransfers.push(dieBKE);
...
void NVM_PHY_ONFI_NVDDR2::Execute_simulator_event(MQSimEngine::SimEvent* ev)
{
  ...
  if (!WaitingCopybackWrites[channel_id].empty())
  {
    ...
    sim->Register_sim_event(sim->Time() + this->__spec[channel_id].ProgramCommandTime[waitingBKE->__active_transactions.size()],
    ...
    waitingBKE->__expected_finish_time = sim->Time() + this->__spec[channel_id].ProgramCommandTime[waitingBKE->__active_transactions.size()]
...

{
  ...
  case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
    ...
    if (channels[chip.ChannelID].GetStatus() == BusChannelStatus::IDLE)
    {
      ...
      for (auto it = dieBKE.__active_transactions.begin(); it != dieBKE.__active_transactions.end(); it++) {
        (*it)->STAT_transfer_time += __spec[chip.ChannelID].ProgramCommandTime[dieBKE.__active_transactions.size()];
      }
      ...
      auto sim = Simulator;
      sim->Register_sim_event(sim->Time() + __spec[chip.ChannelID].ProgramCommandTime[dieBKE.__active_transactions.size()],
      ...
      dieBKE.__expected_finish_time = sim->Time()
                                        + __spec[chip.ChannelID].ProgramCommandTime[dieBKE.__active_transactions.size()]
