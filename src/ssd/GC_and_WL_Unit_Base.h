  ...
  */
  ...
  public:
    GC_and_WL_Unit_Base(const sim_object_id_type& id, 
      ...
      uint32_t block_no_per_plane, uint32_t page_no_per_block, uint32_t sector_no_per_page,
      bool use_copyback, double rho, uint32_t max_ongoing_gc_reqs_per_plane,
      bool dynamic_wearleveling_enabled, bool static_wearleveling_enabled, uint32_t static_wearleveling_threshold, int seed);
    void Setup_triggers();
    ...
    uint32_t Get_GC_policy_specific_parameter();//Returns the parameter specific to the GC block selection policy: threshold for random_pp, set_size for RGA
    uint32_t Get_minimum_number_of_free_pages_before_GC();
