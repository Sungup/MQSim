...
namespace SSD_Components
{
  ...
  public:
    NVM_Transaction_Flash_RD(Transaction_Source_Type source, stream_id_type stream_id,
      uint32_t data_size_in_byte, LPA_type lpa, PPA_type ppa,
      SSD_Components::UserRequest* related_user_IO_request, NVM::memory_content_type content, NVM_Transaction_Flash_WR* related_write,
      page_status_type read_sectors_bitmap, data_timestamp_type data_timestamp);
    NVM_Transaction_Flash_RD(Transaction_Source_Type source, stream_id_type stream_id,
      uint32_t data_size_in_byte, LPA_type lpa, PPA_type ppa, const NVM::FlashMemory::Physical_Page_Address& address,
      SSD_Components::UserRequest* related_user_IO_request, NVM::memory_content_type content, NVM_Transaction_Flash_WR* related_write,
      page_status_type read_sectors_bitmap, data_timestamp_type data_timestamp);
    NVM_Transaction_Flash_RD(Transaction_Source_Type source, stream_id_type stream_id,
      uint32_t data_size_in_byte, LPA_type lpa, PPA_type ppa,
      SSD_Components::UserRequest* related_user_IO_request, NVM::memory_content_type content,
      page_status_type read_sectors_bitmap, data_timestamp_type data_timestamp);
