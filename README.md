...
$ mkdir build;
$ cd build;
$ cmake  -DCMAKE_BUILD_TYPE=Release -DFORCE_INLINE=On ..;
$ make $(proc);
$ bin/MQSim -i <SSD Configuration File> -w <Workload Definition File>;
...
Currently not in support for this refactoring version, because I'm not using
the Visual Studio. Also, many files change their name and move their location.
Following instructions is the original MQSim's compiling and running sequences
in Windows.
...
> 4. Run the generated executable file (e.g., MQSim.exe) either in command line
>    mode or by clicking the MS Visual Studio run button. Please specify the
>    paths to the files containing the 1) SSD configurations, and 2) workload
>    definitions.
>
...
18. **Page_Metadata_Capacity:** the size of the metadata area of each physical flash page in bytes. Range = {all positive integer values}.
...
You can define a trace-based workload for MQSim, using the <TraceFlowParameterSet> XML tag. Currently, MQSim can execute ASCII disk traces define in [8] in which each line of the trace file has the following format:
...
You can define a synthetic workload for MQSim, using the <SyntheticFlowParamSet> XML tag.
